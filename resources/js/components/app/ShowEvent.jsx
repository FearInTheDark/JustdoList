...
const ShowEvent = ({event = null, setEvent, setEvents}) => {
    ...
    const pics = useMemo(() => {
        return Array.from({length: 5}, (_) => `https://picsum.photos/seed/${Math.floor(Math.random() * 10000)}/1000`)
    })

    const handleJoin = async () => {
        setLoading(true)
        try {
            await axios.patch(route('events.join', data))
            setJoined(true)
    ...
    const handleUnJoin = async () => {
        setLoading(true)
        try {
            await axios.patch(route('events.leave', data))
            setJoined(false)
            setJoinedCount(pre => pre - 1)
            ...
            <DialogContent className="max-w-task-dialog p-0 dark:bg-slate-800 bg-gray-100">
                ...
                <div className="size-full bg-white dark:bg-slate-800 flex flex-col gap-2 overflow-hidden shadow-2xl rounded-lg p-4">
                    <div className="flex flex-row justify-between tracking-tight pr-10">
                        <div className="flex items-center space-x-2">
                            <a target="_blank" rel="noreferrer">
                                <img title="Profile picture of Dillion" alt="dillionverma" height="48" width="48" src={event?.author?.image ? `/storage/uploads/${event?.author?.image}` : "https://twitter.com/dillionverma/status/1678577280489234432"}
                    ...
                    <div className={`relative flex transform-gpu snap-x snap-mandatory gap-4 overflow-x-auto`}>
                        ...
                        {/*{event && JSON.parse(event?.images).map((image, index) =>*/}
                        {/*    <img src={image} alt="Image" key={index} className="h-64 w-5/6 shrink-0 snap-center snap-always rounded-xl border object-cover shadow-sm"/>)}*/}
                        {/*{event && JSON.parse(event?.images).map((image) => <img src={`https://picsum.photos/seed/${Math.floor(Math.random() * 10000)}/1000`} alt="Image" key={image} className="h-64 w-5/6 shrink-0 snap-center snap-always rounded-xl border object-cover shadow-sm"/>)}*/}
                        {event && Array.from({length: 5}, (_, i) =>
                            <img src={pics.pop()} key={i}
