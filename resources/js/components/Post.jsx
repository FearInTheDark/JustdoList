...
const Post = ({event = {}, onClick}) => {
    return (
        <div onClick={onClick} data-aos="zoom-out"
            ...
            <div className={`relative flex transform-gpu snap-x snap-mandatory gap-4 overflow-x-auto`}>
                ...
                {event && JSON.parse(event?.images).map((image, index) =>
                    <img src={`/storage/app/events/${image}`}
                    alt="Image" key={index} className="h-64 w-5/6 shrink-0 snap-center snap-always rounded-xl border object-cover shadow-sm"/>)}
                <div className="shrink-0 snap-center sm:w-2"></div>
            </div>
        </div>
    );
