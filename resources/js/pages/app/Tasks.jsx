...
const Tasks = () => {
    const {props} = usePage()
    const initTasks = props.user.tasks
                ...
                <div className="mb-4 flex space-x-2">
                    {['title', 'incomingDate', 'priority'].map((key) => (
                        ...
                        >
                            {key}
                            {sortConfig?.key === key && (
                                sortConfig.direction === 'asc' ? <ChevronUp className="ml-2 h-4 w-4"/> :
                                    ...
                                    <Draggable key={task.id} draggableId={task.title} index={index}>
                                        {(provided) => (
                                            <div
                                                ...
                                                className={`bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 transition-all duration-300 ease-in-out hover:shadow-lg ${layout === 'pinterest' ? 'mb-4 break-inside-avoid' : ''}`}
